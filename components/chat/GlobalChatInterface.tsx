...
export default function GlobalChatInterface({ user }: GlobalChatInterfaceProps) {
    ...
    const loadMessages = async () => {
      try {
        ...
        if (userIds.length > 0) {
          ...
          console.log('GlobalChatInterface: employees 데이터:', employees);
          (employees || []).forEach((emp: any) => {
            if (emp.auth_id) {
              console.log(`매칭: ${emp.auth_id} -> ${emp.name}`);
              map[emp.auth_id] = {
                name: emp.name,
                email: emp.auth_id
              };
          ...
          console.log('GlobalChatInterface: 생성된 userMap:', map);
          setUserMap(map);
        }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
        {messages.map((message, index) => {
          ...
          console.log(`메시지 ${index}: user_id=${message.user_id}, senderInfo=`, senderInfo, ', displayName=', displayName);
