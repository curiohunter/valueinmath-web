...
export default function LearningHistoryPage() {
  ...
  }, [dateRange.from, filteredClassId, filteredStudentId]);
  ...
  async function fetchLogs() {
    ...
    try {
      let query = supabaseClient
        .from("study_logs")
        .select("class_id,student_id,date,attendance_status,homework,focus,book1,book1log,book2,book2log,note");
  ...
  async function handleSaveEdit(edited: any) {
    ...
    try {
      const { error } = await supabaseClient
        ...
        .update({
          date: edited.date,
          attendance_status: edited.attendance_status,
          homework: edited.homework,
          focus: edited.focus,
          book1: edited.book1,
          book1log: edited.book1log,
          book2: edited.book2,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 p-3 rounded-lg">
        <div className="text-sm text-blue-800">
          ...
          {!filteredClassId && !filteredStudentId && ` | 전체`}
        </div>
      </div>
                ...
                <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-lg p-4">
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="space-y-3">
                      <label className="block text-sm font-medium text-gray-700">출결 상태</label>
                      <div className="space-y-2">
                        ...
                        {editingRow.attendance_status && (
                          <div className="flex justify-center">
                            <span className={`inline-flex items-center justify-center w-10 h-10 text-sm font-bold rounded-full border-2 ${scoreColor(parseInt(editingRow.attendance_status))}`}>
                              {editingRow.attendance_status}
                            </span>
                          </div>
                        )}
                    ...
                    {/* 숙제 */}
                    <div className="space-y-3">
                      <label className="block text-sm font-medium text-gray-700">숙제 완료</label>
                      <div className="space-y-2">
                        <select 
                          className="w-full h-11 px-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white"
                        ...
                        {editingRow.homework && (
                          <div className="flex justify-center">
                            <span className={`inline-flex items-center justify-center w-10 h-10 text-sm font-bold rounded-full border-2 ${scoreColor(parseInt(editingRow.homework))}`}>
                              {editingRow.homework}
                            </span>
                          </div>
                        )}
                    ...
                    {/* 집중도 */}
                    <div className="space-y-3">
                      <label className="block text-sm font-medium text-gray-700">수업 집중도</label>
                      <div className="space-y-2">
                        <select 
                          className="w-full h-11 px-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white"
                        ...
                        {editingRow.focus && (
                          <div className="flex justify-center">
                            <span className={`inline-flex items-center justify-center w-10 h-10 text-sm font-bold rounded-full border-2 ${scoreColor(parseInt(editingRow.focus))}`}>
                              {editingRow.focus}
                            </span>
                          </div>
                        )}
              ...
              <div className="space-y-4">
                ...
                <div className="bg-purple-50 rounded-lg p-4 space-y-4">
                  ...
                  <div className="bg-white rounded-lg p-4 border border-purple-100">
                    <div className="text-sm font-medium text-purple-700 mb-3">📚 교재 1</div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">교재명</label>
                        <Input 
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">진도 내용</label>
                        <Input 
                  ...
                  {/* 교재 2 */}
                  <div className="bg-white rounded-lg p-4 border border-purple-100">
                    <div className="text-sm font-medium text-purple-700 mb-3">📖 교재 2</div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">교재명</label>
                        <Input 
