...
## 글로벌 채팅(학원 전체 채팅) 기능 완성 (2025.06.03)
...
- [x] 일정 등록 모달 UI/UX 대폭 개선 (2025.05.27)
  - [x] 일정유형/상태를 제목 위로 이동, 정보 입력 순서 개선
  - [x] 담당관 선택 UI를 담당자와 동일한 카드 스타일로 통일
  - [x] 시작일시 변경 시 종료일시 1시간 후로 자동 설정(24시 넘으면 다음날)
  - [x] 학생 선택 기능 완전 제거(상태/함수/DB연결/폼리셋/UI 등 전체)
  - [x] 코드/상태/폼/DB연동/UX 전반 리팩토링 및 불필요 코드 삭제
...
- ✅ 일정 등록 모달에서 학생 선택 기능 완전 제거 및 UI/UX 단순화(2025.05.27)
- ✅ 일정유형/상태 UI 위치 개선, 담당관 UI 통일, 시작-종료일시 자동화 등 실사용성 대폭 향상(2025.05.27)
...
## 마지막 업데이트
- 날짜: 2025년 5월 27일
- 작업: 일정 등록 모달 UI/UX 대폭 개선(일정유형/상태 위치, 담당관 UI 통일, 시작-종료일시 자동화, 학생 선택 완전 제거 등). 코드/상태/폼/DB연동/UX 전반 리팩토링 및 불필요 코드 삭제. 다음 작업은 학습 관리 페이지 구현 예정.
...
## 최근 주요 이슈 및 해결 내역 (2025.05)

### [수업 일정 관리] 캘린더 네비게이션/상태 관리 문제 해결
- react-big-calendar 도입 후, 월/주/일, 이전/다음, 오늘 버튼이 동작하지 않는 문제 발생
- 원인: Calendar 컴포넌트에 date, view, onNavigate, onView 상태/핸들러가 누락되어 정적으로 고정됨
- 해결: useState로 date/view 상태 선언, useCallback으로 onNavigate/onView 핸들러 구현, Calendar에 props로 전달
- 모든 네비게이션/뷰 전환/이벤트 표시 정상 동작 확인
- 실무 팁: 외부 UI 라이브러리는 상태 관리가 없으면 동작이 제한됨, 버튼/뷰 전환이 안 되면 상태/핸들러 누락을 의심할 것
- 상세 내역은 LEARNED_FROM_ERRORS.md 참고