-- 월별 통계 스냅샷 저장 함수
-- 수정: 2025-10-31 - COUNT(*) → SUM(dept_count) 버그 수정
...
BEGIN
  ...
  -- 재적 학생 집계 (SUM 사용으로 수정)
  SELECT
    SUM(dept_count),
    jsonb_object_agg(
      COALESCE(department, '미지정'),
      dept_count
    )
  ...
  -- 이번달 신규 등록 집계 (SUM 사용, start_date 필드 사용)
  SELECT
    SUM(dept_count),
    jsonb_object_agg(
      COALESCE(department, '미지정'),
  ...
  FROM (
    ...
    WHERE s.start_date >= date_trunc('month', CURRENT_DATE)
      ...
      AND s.status IN ('재원', '휴원')
    GROUP BY s.department
  ) AS enroll_stats;

  -- 이번달 퇴원 집계 (SUM 사용, end_date 필드 사용)
  SELECT
    SUM(dept_count),
    jsonb_object_agg(
      COALESCE(department, '미지정'),
  ...
  FROM (
    SELECT
      ...
      COUNT(*) as dept_count
    FROM students s
    WHERE s.end_date >= date_trunc('month', CURRENT_DATE)
      AND s.end_date < date_trunc('month', CURRENT_DATE) + interval '1 month'
      AND s.status = '퇴원'
    GROUP BY s.department
  ) AS withdraw_stats;
